...
export function TreePage() {
  ...
  const [traversal, setTraversal] = useState<TraversalKey>('inorder')
  ...
  function run() {
    ...
    if (traversal === 'inorder') newSteps = inorder(tree)
    else if (traversal === 'preorder') newSteps = preorder(tree)
    else newSteps = postorder(tree)
  ...
  function loadRandomTree() {
    const values = Array.from({ length: 7 + Math.floor(Math.random() * 4) }, () =>
  ...
  const isComplete = hasRun && currentStepIndex === steps.length - 1
    ...
    <div className="min-h-screen text-ink font-body">
      ...
      <main className="max-w-6xl mx-auto px-8 py-12">
        ...
        <div className="border border-panel-border bg-panel p-5 font-mono text-xs text-ink-faint grid grid-cols-3 gap-x-4 gap-y-3 max-w-md mb-10">
          <div>
            <b className="block text-ink-dim text-[10px] tracking-wide uppercase mb-1">Traversal</b>
            <span className="text-ink text-[13px]">{info.name}</span>
          </div>
          <div>
            <b className="block text-ink-dim text-[10px] tracking-wide uppercase mb-1">Order</b>
            <span className="text-ink text-[13px]">{info.order}</span>
          </div>
          <div>
            <b className="block text-ink-dim text-[10px] tracking-wide uppercase mb-1">Status</b>
            <span className="text-accent-2 text-[13px]">
              {isComplete ? 'Complete' : isPlaying ? 'Running' : 'Idle'}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-start justify-between mb-8 flex-wrap gap-4">
          <div>
            ...
            <p className="text-ink-dim max-w-md">{info.description}</p>
            <div className="flex gap-3 mt-3 flex-wrap">
              {[
                ...
                <span key={key} className="flex items-center gap-1.5 font-mono text-[11px] text-ink-faint">
                  <kbd className="border border-panel-border bg-panel px-1.5 py-0.5 rounded text-ink-dim">{key}</kbd>
                  {label}
                </span>
              ))}
            </div>
          ...
          <div className="flex flex-col gap-1">
            <label className="font-mono text-[10px] tracking-wide uppercase text-ink-dim">Traversal</label>
            <select
              value={traversal}
              onChange={(e) => { setTraversal(e.target.value as TraversalKey); reset() }}
        ...
        <div className="border border-panel-border bg-panel relative">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px]">
            ...
            <div className="p-6 border-b lg:border-b-0 lg:border-r border-panel-border">
              <TreeRenderer root={tree} currentStep={currentStep} />
              ...
              <div className="flex flex-col gap-1.5 mt-5 font-mono text-sm">
                ...
                <p className="text-xs text-ink-faint">
                  Comma-separated integers, 1–999, up to 15 values. Inserted in order to form a binary search tree. Duplicates are ignored.
                </p>
              </div>

              {/* Playback controls */}
              <div className="flex items-center gap-3 mt-4 font-mono text-sm flex-wrap">
                <button onClick={reset} className="w-9 h-9 border border-panel-border rounded hover:border-accent transition">↺</button>
                <button
                  onClick={() => { if (!hasRun) run(); else setIsPlaying(p => !p) }}
                  className={`w-9 h-9 border rounded transition ${isPlaying ? 'bg-accent border-accent text-bg-deep' : 'border-panel-border hover:border-accent'}`}
                >
                  {isPlaying ? '⏸' : '▶'}
                </button>
                <button
                ...
                <label className="flex items-center gap-2 ml-2 text-ink-dim">
                  speed
                  <input type="range" min={1} max={5} value={speed} onChange={(e) => setSpeed(Number(e.target.value))} className="accent-accent" />
                </label>
                <button
                  onClick={() => { reset(); setTimeout(run, 50) }}
              ...
              {hasRun && currentStep && (
                <div className="flex gap-7 mt-5 font-mono text-xs text-ink-dim">
                  <div>
                    <span className="block text-2xl font-display text-ink">{currentStep.visitedOrder.length}</span>
                    nodes visited
                  </div>
                  <div>
              ...
              {isComplete && (
                <div className="mt-4 px-4 py-3 border border-accent-2/40 bg-accent-2/10 text-accent-2 font-mono text-sm">
                  ✓ Traversal complete — order: {currentStep?.visitedOrder.map(id => findValue(tree, id)).join(', ')}
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col h-full min-h-[400px]">
              <div className="flex items-center justify-between px-4 py-3 border-b border-panel-border bg-panel">
                <span className="font-mono text-[11px] tracking-widest uppercase text-ink-dim">{info.name}</span>
                <span className="font-mono text-[10px] text-ink-faint">line {activeLine + 1} / {info.code.length}</span>
              </div>
                  ...
                  return (
                    <div
                      ...
                      className={`flex gap-4 px-3 py-1 transition-all duration-150 border-l-2 ${
                        isActive ? 'bg-accent/10 border-accent' : 'border-transparent hover:bg-panel/40'
                      }`}
                    >
                      <span className={`select-none w-4 text-right shrink-0 text-xs mt-0.5 ${isActive ? 'text-accent' : 'text-ink-faint'}`}>
                        {index + 1}
                      </span>
                      <span className={isActive ? 'text-ink' : 'text-ink-dim'}>{highlightLine(line)}</span>
                    </div>
                  )
                })}
              </div>
              <div className="px-4 py-3 border-t border-panel-border bg-panel min-h-[56px] flex items-center gap-3">
                ...
                <p className="font-mono text-xs text-ink-dim leading-relaxed">
                  {!hasRun && 'Press Run to begin the traversal.'}
                  {hasRun && currentStep && (
                    <>Visiting node value <b className="text-ink">{currentStep.current !== null ? findValue(tree, currentStep.current) : '—'}</b></>
                  )}
                </p>
              </div>
            </div>
